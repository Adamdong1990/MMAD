CZIGPFx <- function(phi0, phi, la, th)
{
  ...
  for(i in 1:m)
  {
    ...
    esum1 = (1-r1) - esum0
    df[i+1,i+1] = -(1-phi0)^2*(1-exp(-la[i]))^2*a[i]^2/r1- esum0*((1-exp(-la[i]))^2)/(phi[i]+(1-phi[i])*exp(-la[i]))^2 - esum1/((1-phi[i])^2)
    df[i+m+1,i+m+1] = (1-phi0)*(1-phi[i])*exp(-la[i])*a[i] - (1-phi0)^2*(1-phi[i])^2*exp(-2*la[i])*a[i]^2/r1 + esum0*phi[i]*(1-phi[i])*exp(-la[i])/(phi[i]+(1-phi[i])*exp(-la[i]))^2 + (1-phi0)*(1-phi[i])*(th[i]/(la[i]+2*th[i])-1/la[i])
    df[i+2*m+1,i+2*m+1] = -(1-phi0)*(1-phi[i])*( 2*la[i]/(la[i]+2*th[i])+la[i]/(1-th[i]) )

    df[i+m+1,i+1] = df[i+1,i+m+1] = (1-phi0)*exp(-la[i])*a[i] + (1-phi0)^2*(1-exp(-la[i]))*(1-phi[i])*exp(-la[i])*a[i]^2/r1 + esum0*exp(-la[i])/(phi[i]+(1-phi[i])*exp(-la[i]))^2
    df[i+2*m+1,i+1] = df[i+1,i+2*m+1] = 0
    df[i+2*m+1,i+m+1] = df[i+m+1,i+2*m+1] = -(1-phi0)*(1-phi[i])*la[i]/(la[i]+2*th[i])
  }
...
CZIGPQx <- function(phi0, phi, la, th)
{
  ...
  dphi = -(1-r1-esum0)/(1-phi)^2 + sum((1-phi0)*a0*(-1/be*phi-(1-phi/be)/(1-phi)^2 )) + esum0*(-1/be*phi+(1-phi/be)/(1-phi)^2)
  dla = -(1-phi0)*(1-phi)/la
