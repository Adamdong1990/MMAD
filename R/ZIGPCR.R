...
ZIGPFx <- function(phi0, la, th)
{
  ...
  for(i in 1:m)
  {
    ...
    df[i+1,i+1] = -(1-phi0)/la[i] + th[i]*(1-phi0)/(la[i]+2*th[i]) + phi0*(1-phi0)*a0/r1
    df[i+m+1,i+m+1] = -la[i]*(1-phi0)/(1-th[i]) - 2*la[i]*(1-phi0)/(la[i]+2*th[i])
    df[i+m+1,i+1] = df[i+1,i+m+1] = -la[i]*(1-phi0)/(la[i]+2*th[i])
  }
