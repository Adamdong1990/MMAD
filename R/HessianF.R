...
HessianF <- function(N, q, x, d, vd, a, b, lambda, theta, beta, vy)
{
  ...
  df[1,1] = sum( rowSums(d)/(theta^2) + 2*digamma(1/theta)/(theta^3) + trigamma(1/theta)/(theta^4) - 2*digamma(A)/(theta^3) - trigamma(A)/(theta^4)
                 - 2*mc/((theta^2)*(1+theta*mc)) - A*(mc^2)/(1+theta*mc)^2 + (2/(theta^3))*log(1+theta*mc) )
  ...
  for(p in 1:q)
  {
    ...
    df[(p+1),1] = df[1,(p+1)] = sum( rowSums(d)*rowSums(ber*(x[,,p]*La + L_0r_p))/(1+theta*mc)
                                     - (1+theta*rowSums(d))*mc*rowSums(ber*(x[,,p]*La + L_0r_p))/(1+theta*mc)^2 )
    ...
    for (j in 1:q)
    {
      ...
      E_2 = AC*x[,,p]*x[,,j]*exp(apply(x*BE,c(1,2),sum))
