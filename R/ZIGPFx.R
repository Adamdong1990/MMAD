...
#'
#' @examples
#' ZIGPFx(0.5,0.5,1,0.1)
ZIGPFx = function(phi0, phi, la, th)
{
  m = length(phi)
  ...
  df[2:(m+1),1] = df[1,2:(m+1)] = -(1+r1-a0)*(1-exp(-la))*a/r1
  ...
  for(i in 1:m)
  {
    ...
    df[i+m+1,i+m+1] = (1-phi0)*(1-phi[i])*exp(-la[i])*a[i] - (1-phi0)^2*(1-phi[i])^2*exp(-la[i])*a[i]^2/r1 + phi[i]*(1-phi[i])*exp(-la[i])*(1-r1)/(phi[i]+(1-phi[i])*exp(-la[i]))+(1-phi[i])*(1-exp(-la[i]))*(1-r1)*(th[i]/(la[i]+2*th[i])-1/la[i])
