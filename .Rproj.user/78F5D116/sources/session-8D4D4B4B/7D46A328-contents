...
LTNFx <- function(a,mu,sigma)
{
  ...
  df[1,2] = df[2,1] =(a1*dphi -dnorm(a1))/(2*c*sqrt(sigma^3)) + a1*(dnorm(a1))^2/(2*c^2*sqrt(sigma^3))
  df[2,2] = -1/(2*sigma^2) + (a1^2*dphi-a1*dnorm(a1))/(4*c*sigma^2)+a1^2*(dnorm(a1))^2/(4*c^2*sigma^2)
...
LTNQx <- function(a, mu, sigma)
{
  ...
  dQ[2,2] = -(1+s1)/(2*sigma^2) + s1*(a-mu)*g/(sigma^2) - a1*dnorm(a1)/(c*sigma^2)
